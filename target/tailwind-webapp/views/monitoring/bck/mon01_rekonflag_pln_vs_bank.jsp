...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $('#btnExportMonDftAllExcelOneSheet').on('click', async function () {
            ...
            .html('<i class="fa fa-spinner fa-spin"></i> Memuat... (0 data)');

            const { vbln_usulan, vkd_bank, vkd_dist, vproduk } = detailFilterParams;
            ...
            try {
                /* ================= MASTER ================= */
                const namaBank = await fetchNamaBank(vkd_bank);
                const namaUPI  = (vkd_dist === '00')
                    ? '00 - SAKTI'
                    : await fetchNamaUnitUPI(vkd_dist);

                /* ================= HEADER ================= */
                const HEADERS = {
                    PRODUK:'PRODUK', TGLAPPROVE:'TGL APPROVE', KD_DIST:'KD DIST',
                    VA:'VA', SATKER:'SATKER', PLN_NOUSULAN:'PLN NO USULAN',
                    PLN_IDPEL:'PLN IDPEL', PLN_BLTH:'PLN BLTH',
                    PLN_LUNAS_H0:'PLN LUNAS H0', PLN_RPTAG:'PLN RPTAG',
                    PLN_RPBK:'PLN RPBK', PLN_TGLBAYAR:'PLN TGL BAYAR',
                    PLN_JAMBAYAR:'PLN JAM BAYAR', PLN_USERID:'PLN USER ID',
                    PLN_KDBANK:'PLN KD BANK', BANK_NOUSULAN:'BANK NO USULAN',
                    BANK_IDPEL:'BANK IDPEL', BANK_BLTH:'BANK BLTH',
                    BANK_RPTAG:'BANK RPTAG', BANK_RPBK:'BANK RPBK',
                    BANK_TGLBAYAR:'BANK TGL BAYAR', BANK_JAMBAYAR:'BANK JAM BAYAR',
                    BANK_USERID:'BANK USER ID', BANK_KDBANK:'BANK KD BANK',
                    SELISIH_RPTAG:'SELISIH RPTAG', SELISIH_BK:'SELISIH BK',
                ...
                const pageSize = 1000;
                let start = 0, draw = 1;
                let allData = [], totalRecords = 0;

                while (true) {
                    const params = new URLSearchParams({
                        act:'detailData', vbln_usulan, vkd_bank, vkd_dist, vproduk,
                        start, length:pageSize, draw:draw++,
                        'order[0][column]':0, 'order[0][dir]':'asc',
                        'columns[0][data]':'KD_DIST', 'search[value]':''
                    });

                    const res = await fetch(
                        getContextPath() + '/mon-rekon-bankvsperupi',
                    ...
                    json.data.forEach(item => {
                        ...
                        for (const k in HEADERS) {
                            row[k] = ['PLN_RPTAG','PLN_RPBK','BANK_RPTAG','BANK_RPBK','SELISIH_RPTAG','SELISIH_BK']
                                .includes(k)
                                ? Number(String(item[k] || 0).replace(/\./g,'').replace(',','.'))
                                : String(item[k] || '');
                        }
                        allData.push(row);
                    });
                    ...
                    btn.html('<i class="fa fa-spinner fa-spin"></i> Memuat... ('+totalLoaded+'/'+totalRecords+' data)');

                    if (totalLoaded >= totalRecords) break;
                    start += pageSize;
                }
                ...
                /* ===== JUDUL ===== */
                sheet.mergeCells(1,1,1,Object.keys(HEADERS).length + 1);
                sheet.getCell('A1').value = 'MIV REKON DETAIL';
                sheet.getCell('A1').font = { bold:true, size:14 };
                sheet.getCell('A1').alignment = { horizontal:'center', vertical:'middle' };

                sheet.addRow(['BULAN', ': ' + vbln_usulan.substr(4,2)+'/'+vbln_usulan.substr(0,4)]);
                sheet.addRow(['UID/UIW', ': ' + namaUPI]);
                sheet.addRow(['BANK', ': ' + vkd_bank + ' - ' + namaBank]);
                sheet.addRow(['PRODUK', ': ' + vproduk]);
                sheet.addRow(['TOTAL DATA', ': ' + totalLoaded]);
                sheet.addRow(['DICETAK', ': ' + new Date().toLocaleString('id-ID')]);
                sheet.addRow([]);

                // Bold info atas
                for (let r = 1; r <= 7; r++) {
                    sheet.getRow(r).eachCell(c => c.font = { bold:true });
                }

                /* ===== HEADER KOLOM ===== */
                const headerRow = sheet.addRow(['NO'].concat(Object.values(HEADERS)));
                headerRow.height = 28;
                ...
                /* ===== DATA ===== */
                allData.forEach((r,i)=>{
                    const row = sheet.addRow([i+1, ...Object.keys(HEADERS).map(k=>r[k])]);
                    row.eachCell((c,col)=>{
                        c.border = thinBorder();
                        if (col >= 10) c.numFmt = '#,##0';
                    });
                });

                /* ===== ZEBRA ===== */
                sheet.eachRow((row, idx)=>{
                    if (idx > headerRow.number && idx % 2 === 0) {
                        row.eachCell(c=>{
                            c.fill = {
                                type:'pattern', pattern:'solid',
                ...
                saveAs(
                    ...
                    'MIV_REKON_DETAIL_'+namaUPI+'_'+vbln_usulan+'.xlsx'
                );
            ...
            function resetButton(){
                ...
                .html('<i class="fa fa-file-excel"></i> Export Detail Per-UPI');
                spinnerDetail.removeClass('flex').addClass('hidden');
            }
